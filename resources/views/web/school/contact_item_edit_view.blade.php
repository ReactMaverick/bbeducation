<div class="form-group calendar-form-filter">
    ...
    <select class="form-control field-validate-2" name="type_int" id="editContactMethodId">
        ...
        @foreach ($contactMethodList as $key1 => $contactMethod)
            <option value="{{ $contactMethod->description_int }}" {{ $contactItemDetail->type_int==$contactMethod->description_int?'selected':'' }} >
...
<div class="modal-side-field">
    <input type="checkbox" class="" name="receiveInvoices_status" id="editInvoiceContact"
        value="1" {{ $contactItemDetail->receiveInvoices_status=='-1'?'checked':'' }} {{ $contactItemDetail->type_int==1?'':'disabled' }} >
...
<div class="form-group modal-input-field">
    ...
    <input type="text" class="form-control field-validate-2" name="contactItem_txt"
        id="" value="{{ $contactItemDetail->contactItem_txt }}">
</div>

<div class="modal-side-field">
    <label class="form-check-label" for="editSchoolMainId">School Main (not specific
        person)</label>
    <input type="checkbox" class="" name="schoolMainId" id="editSchoolMainId"
        value="1" {{ $contactItemDetail->schoolContact_id==null?'checked':'' }} >
</div>
...
<div class="form-group calendar-form-filter">
    ...
    <select class="form-control" name="schoolContact_id" id="editSchoolContactId">
        ...
        @foreach ($schoolContacts as $key2 => $Contacts)
            ...
            <option value="{{ $Contacts->contact_id }}" {{ $contactItemDetail->schoolContact_id==$Contacts->contact_id?'selected':'' }} >
                {{ $cName }}
            </option>
        @endforeach
    </select>
